...
public class ContainerService {
    ...
    public void fileSave(FileSaveLinfoRequest request){
        try {
            String[] fileNames =null;
            String systemSeparatar = File.separator.toString();
            AppInfoDO appInfo = appInfoRepository.findById(request.getAppId()).orElseThrow(() -> new IllegalArgumentException("can't find job by jobId: " + request.getId()));
            //多文件保存
            for (String path:request.getServerPathFile()) {
                if (Objects.equals(systemSeparatar,"/")){
                ...
                if (originId != null) {
                    ...
                    fileInfo = fileInfoRepository.findById(originId).orElseThrow(() -> new IllegalArgumentException("can't find container by id: " + originId));
                }else {
                    fileInfo = new FileInfoDO();
            ...
            // 获取用户端口
            int port= Math.toIntExact(appInfoService.portApp(request.getAppId()));
            // 文件发给worker
            List<String> workeAddres = request.getWorkeAddres();
            for (String addres: workeAddres) {
                try {
                    System.out.println();
                    String ip =addres.split(":")[0];

                    for (String pat:request.getServerPathFile()) {
                        FileTransferClient client = new FileTransferClient(ip,port,pat); // 启动客户端连接
